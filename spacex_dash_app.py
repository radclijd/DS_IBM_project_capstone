...
def generate_pie_chart(site_dropdown):
    if site_dropdown == 'All':
        fig = px.pie(spacex_df, values='class', names='Launch Site', title='Sucessful Launches for all Sites')
        return fig
    else:
...
def scatter_chart(site_dropdown, payload_slider):
    ...
    if site_dropdown == 'All':
        fig2 = px.scatter(df_scatter, x='Payload Mass (kg)', y='class', color='Booster Version Category')
        return fig2
    else:
        filtered_df = df_scatter[df_scatter['Launch Site']==site_dropdown]
        filtered_df = filtered_df.groupby(['Launch Site'])
